...
describe('WarehouseController', () => {
    ...
    test('getProducts should return a 200 response with products', () => {
        const mockProducts = [new Product('1', 'Product A', 10, 5)];
        mockWarehouseService.getProducts.mockReturnValue(mockProducts);
        
        const response = controller.getProducts();
        
        expect(response.status).toBe(200);
        expect(response.data).toEqual(mockProducts);
    });

    test('getTotalPrice should return a 200 response with the total price', () => {
        mockWarehouseService.getTotalPrice.mockReturnValue(50);
        
        const response = controller.getTotalPrice();
        
        expect(response.status).toBe(200);
        expect(response.data).toBe(50);
    });

    test('findProduct should return a 200 response with the found product', () => {
        const mockProduct = new Product('1', 'Test Product', 10, 5);
        mockWarehouseService.findProduct.mockReturnValue(mockProduct);
        const payload: FindProductPayload = { name: 'Test Product' };
        
        const response = controller.findProduct(payload);
        
        expect(response.status).toBe(200);
        expect(response.data).toEqual(mockProduct);
    });

    test('addProduct should return a 201 response with the new product', () => {
        const mockNewProduct = new Product('2', 'New Product', 20, 1);
        mockWarehouseService.addProduct.mockReturnValue(mockNewProduct);
        const payload: AddProductPayload = { name: 'New Product', price: 20, quantity: 1 };
        
        const response = controller.addProduct(payload);
        
        expect(response.status).toBe(201);
        expect(response.data).toEqual(mockNewProduct);
    });

    test('updateProduct should return a 204 response', () => {
        const payload: UpdateProductPayload = { uuid: 'some-uuid', quantity: 10 };
        
        const response = controller.updateProduct(payload);
